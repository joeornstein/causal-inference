...
************** AIM: This do-file replicates Table A.5 -- Table 3 without covariates 
...
*********************************************************** 			
*********************************************************** 
* TABLE 3 -- without pre-treatment variables
*********************************************************** 
*********************************************************** 

* LAST RUN 17.01.2022
* REpliaction: 26.04.2022
...
* animal protection
		...
		reg animalsprotect_ $tall , robust
				
		sum animalsprotect_ if treat==0
		local m =r(mean)
	outreg2 using "$tables/table3_NO_covariates_appendix.xls", ///
 dec(3) ctitle("animal")  tex label ///
	addstat(Control Mean, `m') nocons keep($tall) bracket replace	 
	
	
* evaluations of political regimes
	...
	foreach var of varlist $dep1part1 {
		...
		*turnout						
		...
		reg willvote19_01 $tall  if regis_dnw==1 | regis_no==1, robust
	...
	* Registration
		...
		reg willreg_01 $tall  if regis_dnw==1 | regis_no==1, robust
