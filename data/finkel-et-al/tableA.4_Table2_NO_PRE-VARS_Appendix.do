**********************************************************************************************************************************
************** PROJECT: Can Online Civic Education Induce Democratic Citizenship? Experimental Evidence from a New Democracy 
************** AIM: This do-file replicates Table A.4 -- Table 2 without covariates 
...
****************************
	...
	regis_dnw employed   polint_new_2 polint_new_3 ///
	 polint_new_4  dem_pref autoc_pref  ///z
...
**** Aggregated treatment dummies
		...
		* Part 1 for Latex
		...
		reg animalsprotect_01  treat, robust
		sum animalsprotect_01 if treat==0
		local m =r(mean)
			outreg2 using "$tables/table2_NO_covariates_part1_appendix.xls", ///
			dec(3) ctitle("animal")  tex label ///
			addstat(Control Mean, `m') ///
			nocons keep(treat) bracket replace	 
			 ...
			 foreach var of varlist $dep1part1 {
			  ...
			  local m =r(mean)
				outreg2 using "$tables/table2_NO_covariates_part1_appendix.xls", ///
					...
					addstat(Control Mean, `m') ///
					nocons keep(treat) bracket append 	
		...
		* Part 2 for Latex			
		...
		reg willvote19_01 treat, robust
		sum willvote19_01 if treat==0
		local m =r(mean)
			outreg2 using "$tables/table2_NO_covariates_part2_appendix.xls", ///
			dec(3) ctitle("willvote19_01")  tex label ///
			addstat(Control Mean, `m') ///
			nocons keep(treat) bracket replace
			
			
		reg willvote19_01 treat if regis_dnw==1 | regis_no==1, robust
		...
		reg willreg_01 treat if regis_dnw==1 | regis_no==1, robust
		...
		reg nonvotepar_01 treat, robust
